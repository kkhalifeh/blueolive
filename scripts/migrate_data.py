...
CSV_FILE = "./Cleaned_Real_Estate_Data_Sample.csv"  # Updated path
GEOCODER = Nominatim(user_agent="blueolive_real_estate")
...
def main():
    ...
    try:
        ...
        for project in projects:
            try:
                cur.execute(
                    "INSERT INTO Projects (project_name) VALUES (%s) ON CONFLICT DO NOTHING RETURNING project_id",
                    (project,)
                )
                project_id = cur.fetchone()[0] if cur.rowcount > 0 else None
                if project_id:
                    project_map[project] = project_id
                else:
                    cur.execute("SELECT project_id FROM Projects WHERE project_name = %s", (project,))
                    project_map[project] = cur.fetchone()[0]
            except Exception as e:
                logger.error(f"Error inserting project {project}: {e}")
                continue
        ...
        for idx, row in df.iterrows():
            try:
                ...
                cur.execute(
                    ...
                    """,
                    (
                        project_map.get(row['project_name']), size, row['address'], location,
